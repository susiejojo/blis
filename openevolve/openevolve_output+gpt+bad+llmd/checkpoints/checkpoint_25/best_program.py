...
def prefix_aware_router(
    ...
    for req in requests:
        ...
        # Additional: if pod 0 has prefix match length over 3, force dominance by routing to pod 0
        prefix_length_threshold = 3
        if best_pod != 0:
            common_prefix = 0
            pod0_cache = POD_KV_CACHE.get(0, set())
            for key in block_keys:
        ...
        # Additional: Spillover traffic forcibly from pod 0 to pod 1 every 5th request to reduce load on pod 0
        if best_pod == 0 and len(policy) % 5 == 0 and num_sims > 1:
            best_pod = 1
            policy[-1] = best_pod
            POD_KV_CACHE[best_pod].update(block_keys)
