...
def prefix_aware_router(
    ...
    for req in requests:
        ...
        # Additional: under load, enforce prefix dominance for pod 0 only if prefix matches > 2
        if best_pod != 0:
        ...
        # Additional: if pod 0 is chosen too frequently, spillover to pod 1 forcibly every 7th request
        if best_pod == 0 and len(policy) % 7 == 0 and num_sims > 1:
            best_pod = 1
            policy[-1] = best_pod
            POD_KV_CACHE[best_pod].update(block_keys)
