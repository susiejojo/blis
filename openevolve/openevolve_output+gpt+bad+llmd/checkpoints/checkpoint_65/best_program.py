...
def prefix_aware_router(
    ...
    for idx, req in enumerate(requests):
        ...
        for key in block_keys:
            if key in pod0_cache:
                ...
                break
        # Introduce random adjustment to threshold and invert condition occasionally to flip dominance
        if current_pod != 0:
            adjusted_threshold = prefix_length_threshold + (idx % 4 == 0)
            if (common_prefix >= adjusted_threshold) != (idx % 5 == 0):
                # Flip dominance override on some requests to diversify routing
                policy[idx] = 0
    ...
    # Additional: spillover traffic forcibly from pod 0 to pod 1 every 3rd request (increase spill frequency)
    if num_sims > 1:
        for i in range(len(policy)):
            if policy[i] == 0 and (i + 1) % 3 == 0:
                tokens = tokenize_prompt(requests[i].input, tokenizer)
                block_keys = tokens_to_kv_block_keys(tokens, kv_block_size)
                # Alternate spillover pod between pod 1 and pod 2 (if exists) for load distribution
