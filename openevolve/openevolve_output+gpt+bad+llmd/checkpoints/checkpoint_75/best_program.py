        ...
        if current_pod != 0:
            ...
            for key in block_keys:
                if key in pod0_cache:
                    ...
                    break
            # Add randomness to threshold: increase or decrease threshold by 1 based on index mod 4
            adjusted_threshold = prefix_length_threshold + ((idx % 4) - 2) // 2
            if common_prefix >= adjusted_threshold:
                # Override to pod 0 to enforce prefix dominance when prefix is long enough
                policy[idx] = 0
                POD_KV_CACHE[0].update(block_keys)

    # Additional: spillover traffic forcibly from pod 0 to pod 1 every 2nd request (increase spill frequency)
    if num_sims > 1:
        for i in range(len(policy)):
            if policy[i] == 0 and (i + 1) % 2 == 0:
