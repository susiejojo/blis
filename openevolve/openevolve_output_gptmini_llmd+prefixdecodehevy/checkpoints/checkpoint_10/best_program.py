...
def prefix_aware_router(
    ...
    for req in requests:
        ...
        # Spillover: If best pod has more than 80% of total KV cache size among all pods,
        # spillover to round-robin for fairness
        total_cache_size = sum(len(c) for c in POD_KV_CACHE.values())
        if total_cache_size > 0:
            dominant_ratio = len(POD_KV_CACHE[best_pod]) / total_cache_size
            if dominant_ratio > 0.8:
                best_pod = (best_pod + 1 + len(policy)) % num_sims
                # Update again with new pod to keep cache balanced
