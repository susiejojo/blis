...
def prefix_aware_router(
    ...
    for req in requests:
        ...
        # Spillover: If best pod has more than 75% of total KV cache size among all pods,
        # forcibly route to next pod (circular) to force load spillover and reduce latency spikes
        total_cache_size = sum(len(c) for c in POD_KV_CACHE.values())
        if total_cache_size > 0:
            dominant_ratio = len(POD_KV_CACHE[best_pod]) / total_cache_size
            if dominant_ratio > 0.75:
                # Choose next pod skipping one to increase spillover randomness
                best_pod_alt = (best_pod + 2 + len(policy)) % num_sims
                # Update with new pod to keep cache balanced
                POD_KV_CACHE[best_pod_alt].update(block_keys)
                policy[-1] = best_pod_alt
